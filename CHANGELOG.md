...
### Security
- `data_view` (modal "Ver datos") y `progress_view` ahora exigen permiso de
  vista sobre ImportJob además de `is_staff`: en instalaciones con staff
  limitado (p. ej. vendedores con acceso al admin), cualquier staff podía
  leer por URL directa el contenido del archivo importado (datos personales)
  y el progreso/errores de cualquier job.
